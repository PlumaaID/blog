...
Muchos autores en el tema utilizan la metáfora de la *[huella digital](https://learn.microsoft.com/es-es/purview/sit-document-fingerprinting)* para referirse a los identificadores únicos de documentos (resultado de [funciones hash](https://en.wikipedia.org/wiki/Hash_function)), el problema es que la huella digital es una conceptualización que no le hace justicia al funcionamiento de un hash, o *función resumen*.
...
Adicional a eso, si el mensaje de datos a firmar se acompaña con un certificado NOM-151 y una consulta firmada por el SAT en respecto a la validez del certificado, entonces mantedrá toda la solidez jurídica para demostrar su validez.
